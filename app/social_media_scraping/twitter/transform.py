...
def transform(extracted) -> pd.DataFrame:
    ...
    df_combined['mentionedUsers'] = df_combined['mentionedUsers'].apply(lambda x: list(pd.DataFrame(x)['username']) if str(x) != '[]' else None)
    ...
    df_combined['photoLinks'] = df_combined['media'].apply(lambda x: list(pd.DataFrame(x['photos'])['url']) if str(x['photos']) != '[]' else None)
    df_combined['videoLinks'] = df_combined['media'].apply(lambda x: [pd.DataFrame(x['videos'])['variants'][0][0]['url']] if str(x['videos']) != '[]' else None)
    df_combined['animatedLinks'] = df_combined['media'].apply(lambda x: list(pd.DataFrame(x['animated'])['thumbnailUrl']) if str(x['animated']) != '[]' else None)
