...
def load(transformed_tweets):
    for tweet in transformed_tweets.values():
        try:
            ...
            values = (tweet["content"],tweet["date"], tweet["user"], tweet["comment"], tweet["share"],
                      tweet["likes"],tweet["image"], tweet["scrapingTimeStamp"])
            database = Database()
            ...
            if "duplicate key value violates unique constraint" in str(e):
                print(
                    "Tweet already exists in the database.")
