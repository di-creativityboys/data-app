        ...
        try:
            database.execute(
                ...
                (
                    ...
                    [pd.DataFrame(row.media["videos"])["variants"][0][0]["url"]]
                    if str(row.media["videos"]) != "[]"
                    else None,  # videoLinks
                    list(pd.DataFrame(row.media["animated"])["thumbnailUrl"])
                    if str(row.media["animated"]) != "[]"
                    else None,  # animatedLinks
                ),
            )
