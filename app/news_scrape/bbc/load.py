...
def load(transformed_articles):
  for article in transformed_articles.values():
   try:
    ...
    values=(article["url"], article["headline"], article["contents"], article["authors"], article["date"],article["ImageURL"], article["ImageDescription"], article["scrapingTimeStamp"], article["source"], article["topics"])
    database=Database()
